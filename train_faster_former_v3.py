...
def cal_decoder_student_similarity_performance(dec_output, all_highway_exits):
    dec_output = dec_output[:, -1, :]

    total_loss = 0.0
    for early_exit_output in all_highway_exits:
        loss_function = nn.CosineEmbeddingLoss()
        early_exit_output = early_exit_output[:, -1, :]
        loss = loss_function(dec_output, early_exit_output, Variable(torch.Tensor(dec_output.size(0)).cuda().fill_(1.0)))
        total_loss += loss
    return total_loss
...
def train_epoch(model, training_data, optimizer, opt, device, smoothing, training_mode):
    ...
    for batch in tqdm(training_data, mininterval=2, desc=desc, leave=False):
        ...
        if training_mode == TRAIN_BASE:
            ...
            enc_output, all_highway_exits, _ = model(src_seq, trg_seq, train_encoder_exit=True)
        elif training_mode == TRAIN_DECODER:
            dec_output, all_highway_exits, _ = model(src_seq, trg_seq, train_decoder_exit=True)
        ...
        if training_mode == TRAIN_BASE:
            ...
            loss = cal_decoder_student_similarity_performance(dec_output, all_highway_exits)
